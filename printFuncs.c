...
void printMetadata(char *name){
	...
	printf( "%s \n", XML);
	printf( "%s \n", GPX);
	printf( "%s %s \n %s %s %s %s \n %s %s %d - %d - %d %s %d : %d : %d %s %s \n %s %s \n %s %s \n %s %s \n",
		SPC,META,
		SPC2,NAME,name,NAME_C,
		SPC2,TIME,(*timeinfo).tm_year + YEAR_DIFF,(*timeinfo).tm_mon + MON_DIFF,(*timeinfo).tm_mday,
		T,(*timeinfo).tm_hour,(*timeinfo).tm_min,Z,TIME_C,(*timeinfo).tm_sec,
		SPC,META_C,
		SPC,TRK,
		SPC2,TRKSEG);
}
